...
router.post("/", async (req, res) => {
  ...
  console.log("[POST /api/routes] body:", req.body);
  console.log("[POST /api/routes] values:", values);
  try {
    ...
    console.log("[POST /api/routes] insertId:", result.insertId);
    const [rows]: any = await pool.query("SELECT * FROM route_optimization WHERE id = ?", [result.insertId]);
    console.log("[POST /api/routes] created row:", rows[0]);
