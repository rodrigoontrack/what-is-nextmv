...
const HistoryPage = () => {
  ...
  useEffect(() => {
    const loadPickupPoints = async () => {
      try {
        ...
        setPickupPoints(data.map((p: any) => ({
          id: String(p.id),
          name: p.address || `${p.latitude},${p.longitude}`,
          address: p.address || "",
  ...
  const loadRuns = async () => {
    ...
    try {
      ...
      const runsList = (optimizationsData || []).map((opt: any) => ({
        ...
        metadata: { created_at: opt.created_at, id: String(opt.id), status: "succeeded" },
        created_at: opt.created_at,
        status: "succeeded",
        optimization_result: opt.optimization_result,
      }));
      setRuns(runsList);
    } catch (error) {
  ...
  const handleRunSelect = async (runId: string) => {
    ...
    try {
      const optimizationData = await getOptimization(Number(runId));
      setSelectedRunData(optimizationData);
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ID: {selectedRunId} | {selectedRunData?.created_at &&
                      new Date(selectedRunData.created_at).toLocaleString('es-ES', {
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span>Ejecuciones Anteriores</span>
              </div>
              <Button
                onClick={loadRuns}
                variant="outline"
                        ...
                        (() => {
                          ...
                          const stopsWithDetails = vehicleRoute
                            ...
                            .map((routeStop: any) => {
                              ...
                              let point = pickupPoints.find(p => p.id === originalPointId);
